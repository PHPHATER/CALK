<template>
  <div style="margin-top: 138px;">
    ...
    <section>
      ...
      <el-form
        :model="user"
        @submit.prevent.native="join('user')"
        v-if="!joined"
        :rules="rules"
        ref="user"
      >
    ...
    <section class="sms-grid" v-if="joined">
      <el-tag
        ...
        :key="message.id"
        closable
        @close="removeSMS(message.id)"
      >{{ message.text }}</el-tag>
    </section>
    <el-form
      :model="smsHolder"
      @submit.prevent.native="sendSMS('smsHolder')"
      v-if="joined && formExist"
      :rules="rules2"
      ref="smsHolder"
      class="smsbottom"
    >
    ...
    return {
      ...
      rules: {
        name: [
          {
            required: true,
            message: "Впиши свое поганяло, пидор!",
            trigger: "blur"
          },
          {
            min: 3,
            max: 40,
            message: "Длина поганяла от 3х до 40 символов, пидор!",
            trigger: "blur"
          }
        ],
        position: [
